...
// Постоянные факты без даты, что тонут в зелёном «можно ехать» — см.
// countryRowHTML(). Ключ — код страны, значение — короткая подпись.
const PERMANENT_WARNING = {
  PL: 'Варшава', NO: 'Цепи', AT: 'Ночной запрет',
  // Добавлены 14.08.2026 вместе с новыми постоянными фактами (type: none)
  // в TRUCK_BANS_EU: без этой строки RO и HU светятся зелёным «можно ехать»
  // даже когда постоянное ограничение по факту действует — см. комментарий
  // выше про status()/forDay().
  RO: 'DN1',       // ro-dn1-info: запрет свыше 7,5 т почти каждый день
                    // недели, отдельно от выходного запрета A2/DN7/DN39
  HU: 'Транзит',    // hu-transit-network: трассы 42/55 закрыты транзиту >20 т
};
...
function countryRowHTML(country, iso, isToday) {
  ...
  // Постоянные факты без даты (type: none — Варшава C-16, норвежские цепи)
  // не входят в дневной светофор: status() смотрит только forDay(), а у
  // этих строк date пустой. Поэтому страна с таким фактом всё равно
  // светится зелёным «можно ехать», и водитель может не долистать до
  // постоянных ограничений внизу раскрытой карточки. Владелец попросил
  // (12.08.2026, Варшава; 12.08.2026, по аналогии — Норвегия/цепи) второй
  // строкой подписи цеплять взгляд красным предупреждением именно на
  // зелёном состоянии — сама деталь уже открывается по клику на страну.
  const warnLabel = PERMANENT_WARNING[country.code];
  const label = warnLabel && state.light === 'ok'
    ? `<span class="bans__label bans__label--stack">
        <span>${esc(state.label)}</span>
        <span class="bans__label-warn">⚠ ${esc(warnLabel)}</span>
      </span>`
...
// ---------- Варшава: закреплённая карточка (12.08.2026, слой S8/S2 с 07.09.2026) --
// Отдельный, всегда видимый блок НАД «Мой маршрут» — по прямой просьбе
// владельца («крайне важное для нас отдельно»). Не завязан на выбранную
// дату или страны маршрута, поэтому не прячем его внутрь обычной карточки
// Польши и не гейтим фильтром маршрута (водитель мог забыть отметить Польшу).
//
// 07.09.2026 владелец уточнил: боевой маршрут парка идёт НЕ через городские
// улицы (зона C-16, там запрет транзита круглосуточный и без исключений —
// «туда мы не едем»), а по экспресс-трассе S8 в объезд города (участок
// Konotopa–Marki, мост Grota-Roweckiego) — это Слой 2 в терминологии
// исследования владельца. На S8 запрет ПОЧАСОВОЙ (07:00–10:00 и
// 16:00–20:00, ежедневно, круглый год), вне этих часов ехать можно свободно.
// Поэтому закреплённая карточка теперь показывает факт `pl-warsaw-s8`, а не
// `pl-warsaw-c16` — последний остался в RULES как верный, но второстепенный
// факт (виден в обычной карточке Польши, `permanent.map(rowDetailHTML)`,
// не пропал). Подробности — TRUCK_BANS_EU/CLAUDE.md, раздел «Варшава:
// боевой маршрут — слой S8/S2, не зона C-16».
//
// Юридический факт (порог, часы, ссылка на источник) приходит из той же
// строки CALENDAR, что и у всех остальных правил (`pl-warsaw-s8` в листе
...
const WARSAW_TITLE = 'Почасовой запрет на S8 (объезд Варшавы)';
...
const WARSAW_VARIANTS = [
  ...
  },
  {
    ...
    steps: [
      ...
      'S2 идёт южнее города, в обход зоны C-16 — без ограничений по времени и бесплатно для любой массы (с 30.12.2022), КРОМЕ грузов ADR.',
      'Дальше по <strong>A2</strong> до съезда на дорогу <strong>№50</strong> в направлении Остроув-Мазовецка.',
      '⚠️ Везёте опасный груз (ADR)? Этот объезд не подходит — на S2 под Урсыновом тоннель категории E, провоз ADR через него запрещён. Берите северный объезд (дороги 50/62).',
    ],
  },
];
...
function paint() {
  ...
  // Весь выбранный период — полными блоками (это то, что нужно прямо
  // сейчас): «Сегодня/Завтра» — 2 дня, «3 дня» — 3, и то, и другое целиком
  // видно сразу. Дальше периода (только у «Своих дат», если выбрали больше
  // 3 дней) — свёрнутыми строками, разворачиваются по нажатию: 11.08.2026,
  // баг, найденный владельцем — при пресете «3 дня» третий день (13.08)
  // обрезался под старый лимит «первые 2 дня» и показывал только сводку без
  // стран, хотя это ещё часть периода, а не «дальше по маршруту».
  const full = days.slice(0, 3)
  ...
  const rest = days.length > 3
    ? `<section class="bans__day">
         ...
         ${days.slice(3).map(daySummaryHTML).join('')}
       </section>`
    : '';
